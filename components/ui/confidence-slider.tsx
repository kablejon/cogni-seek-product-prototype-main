...
export function ConfidenceSlider({ value, onChange, className }: ConfidenceSliderProps) {
  ...
  const getStatus = (val: number) => {
    if (val < 60) return {
      label: isZH ? '印象模糊 (可能是误记)' : 'Vague memory (possibly wrong)',
    ...
    if (val < 90) return {
      label: isZH ? '大概率在此 (比较有把握)' : 'Likely here (fairly confident)',
    ...
    return {
      label: isZH ? '绝对确认 (亲眼所见)' : 'Confirmed (seen with own eyes)',
  ...
  return (
    <div className={cn("space-y-4 select-none", className)}>
      <div className="flex justify-between items-end">
        <div>
          <h3 className="text-white font-bold text-lg">
            {isZH ? '区域置信度校准' : 'Area Confidence Calibration'}
          </h3>
          <p className="text-slate-400 text-xs mt-1">
            {isZH
              ? '拖动滑块，告诉 AI 您对这个判断的把握有多大'
              : 'Drag to tell AI how confident you are about this location'}
          </p>
        </div>
      ...
      <div className="flex justify-between text-[10px] font-mono text-slate-500 uppercase px-1">
        <span>{isZH ? '50% (不确定)' : '50% (UNCERTAIN)'}</span>
        <span>{isZH ? '100% (确认)' : '100% (CONFIRMED)'}</span>
      </div>
    </div>
  )
}
